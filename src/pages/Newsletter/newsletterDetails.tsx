...
export const NewsletterDetails = () => {
  ...
  return (
    <main className="min-h-screen bg-fleece mb-36 md:mb-48">
      <Header 
        title={newsletterDetails ? newsletterDetails.title : "The Rugly Barnacle Blog"}
        img={newsletterDetails?.image || ""}
        tagline={newsletterDetails?.quick_description ||"" }
      />
      ...
      <section className="max-w-4xl mx-auto px-4 py-8">
        ...
        {newsletterDetails ? (
          <article className="bg-white rounded-lg shadow-sm border border-majorelle p-6 md:p-8">
            ...
            <header className="border-b text-center border-majorelle pb-6 mb-6">
              ...
              <div className="flex flex-wrap items-center justify-center gap-4 text-sm text-space_cadet">
                ...
                <div className="flex items-center gap-1">
                  ...
                  <time dateTime={newsletterDetails.created_at}>
                    {formatCartDate(newsletterDetails.created_at)}
                  </time>
                </div>

                {/* Tags */}
                {newsletterDetails.tags && (
                  <div className="flex items-center gap-2">
                    <span className="font-medium">Tags:</span>
                    <div className="flex flex-wrap gap-1">
                      {newsletterDetails.tags.split(',').map((tag, index) => (
                        <span 
                          key={index}
            ...
            {/* Links Section */}
          {newsletterDetails.links && newsletterDetails.links.length > 0 && (
            <footer className="mt-8 pt-6 border-t border-space_cadet grid grid-cols-1 md:grid-cols-2 gap-6 lg:gap-8 ">
                ...
                <div className="lg:pr-4">
                  ...
                  <ul className="space-y-3">
                    {newsletterDetails.links.map((link, index) => (
